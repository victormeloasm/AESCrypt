...
# Função para calcular checksum SHA-256
...
def encrypt_file(file_path, password_bytearray):
    try:
        ...
        checksum_before = calculate_checksum(sanitized_file_path)
        ...
        def encrypt_block(block):
            iv = secrets.token_bytes(16)  # Gera um IV único para cada bloco
        ...
        checksum_after = calculate_checksum(encrypted_file_path)

        messagebox.showinfo("Success", f"File encrypted and original file securely deleted:\n\nChecksum Before: {checksum_before}\nChecksum After: {checksum_after}")
...
# Adição de verificações específicas para grandes arquivos .exe, .zip e .rar
def is_supported_file(file_path):
    supported_extensions = ['.exe', '.zip', '.rar']
...
# GUI principal
root = tk.Tk()
root.title("AESCrypt Argon 7.0AK")  # Nome atualizado
...
# Título principal
title_label = ttk.Label(root, text="AESCrypt Argon 7.0AK", font=('Helvetica', 16))
